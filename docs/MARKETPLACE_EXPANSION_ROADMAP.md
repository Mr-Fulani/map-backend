...
Обновлено: 2026-09-06. Текущая сводка: [OZON_STATUS.md](OZON_STATUS.md).

## Текущее состояние и история этапов

Ozon уже подключён, публикация доступна в AlfaPro. Отдельные дерево/наценки,
подготовка карточки и account-specific drawer реализованы. Справедливый обход и
общие логи выложены в [#317](OZON_OPERATIONS_RELEASE.md), диагностика и уведомления —
в [#318](OZON_HEALTH_RELEASE.md), production baseline `04bb573`.
Автоматизация цен/остатков и заказов у AlfaPro осталась выключенной.

Ниже сохранены границы и evidence **исторических этапов**, преимущественно
2026-08-29–30. Их `DEPLOYED_OFF`, «ключ не создан», отсутствие provider calls и
ранние списки обязательных полей относятся к тому этапу, не к текущему продукту.
Не используйте их как инструкцию снова включить/выключить production.

Из O4 остаются расширенная аналитика, внешний dead-man/test-fire и дальнейший
реальный rollout. Синхронизация FBS-заказов уже имеет отдельный ограниченный
контур; полная история/исполнение/финансовая сверка не завершены и не являются
условием публикации. [Обработка фото/видео](media_processing.md) — другой,
незавершённый пакет, не часть уже закрытого Ozon image preflight.
...
## O4 — цены, остатки, статистика и эксплуатация (исходный состав)

Актуальный статус отдельных частей находится [выше](#текущее-состояние-и-история-этапов)
и в [OZON_STATUS.md](OZON_STATUS.md). Весь O4 нельзя считать ни не начатым,
ни полностью завершённым одним общим статусом.
...
- proration Billing;
- полное исполнение заказов, возвраты/FBO и финансовая сверка Ozon; имеющаяся
  ограниченная синхронизация FBS-заказов описана отдельно в текущем статусе;
