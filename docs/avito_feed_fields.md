...
`SparePartType` (Вид запчасти) Avito определяет сам, если не передан
(напр. фонарь → «Автосвет») — у нас не отправляется.
...
- `Status` = «Remove` — для листингов в статусе archived/deleted (снятие).
