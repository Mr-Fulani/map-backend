...
Красная блокировка опирается только на однозначное машинное правило текущего
user-docs API. Если Avito одновременно отдаёт `required=true` и
`required_by_dependency=true`, поле считается условным и проверяется только по
его `required`-dependency. Текстовые `dependencies_text`, которым не соответствует
структурированное правило, не превращаются в красную ошибку: такой текст может
быть неполным или противоречить самому API.

При контрольном проходе 29 августа 2026 года сохранённое дерево содержало 192
листа. Поля 186 листьев Avito подтвердил HTTP 200; для шести endpoints стабильно
вернулись 400/404: «Шлем», «Кондиционеры и рефрижераторы», «Оснастка и расходные
материалы», «Диагностический и измерительный инструмент», «Для мототехники» и
грузовая «Подвеска и рулевое управление». Для этих шести MAP сохраняет последнюю
схему, но показывает её недостающие поля жёлтым предупреждением и не блокирует
публикацию как доказанной локальной ошибкой.
