...
4. Импорт — **явный шаг** (команда выше), он не врезан в авто-сидинг тенанта.
   Базовый шаблон (`BASE_PART_CATEGORY_TREE`) остаётся источником для
   классификации товаров (там тонкие имена + алиасы), а дерево Avito —
   маркетплейс-структура поверх. Деревья Avito и базовый каталог сосуществуют;
   переклассификацию товаров на новую структуру выполняет тенант.
