...
## Текущая фаза: P1 — observability выделен и проверен локально

P0 завершён отдельным commit. P1 observability выделен из snapshot по hunks и
прошёл локальные gates. Он ещё не выложен: production release, внешние Sentry
dashboards/alerts и период наблюдения остаются отдельными операционными шагами.
P2 не активирован. Остальная новая feed-функциональность заморожена.
...
### P1. Общая production-наблюдаемость — локально verified
