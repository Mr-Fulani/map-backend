...
AI-задача перед генерацией применяет уже известные trusted знания из global graph
к tenant-товару. Поэтому даже если 1С дала только `name/brand/article`, агент
получает конкретные марки/модели/поколения, если они уже накоплены в общей базе.
