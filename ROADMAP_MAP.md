...
## ADDENDUM — Массовые действия с листингами Avito

Этот блок уточняет roadmap после проверки текущего блока `Массовое размещение` в dashboard.

### Проблема

Текущий блок `Массовое размещение` на странице листингов не публикует объявления и не выполняет массовые операции с листингами.
Сейчас он только массово назначает поля размещения: Avito address id, адрес, телефон и контактное лицо.

Поле `Любая категория источника` в этом сценарии не помогает администратору управлять листингами и должно быть убрано из блока массовых действий.

### Цель

Переделать блок в понятный инструмент массового управления листингами:

- [ ] Массово назначать адрес/локацию объявлениям листинга.
- [ ] Массово публиковать выбранные или отфильтрованные листинги.
- [ ] Массово архивировать листинги, то есть временно снимать объявления с публикации.
- [ ] Массово удалять листинги там, где удаление допустимо бизнес-правилами и статусом объявления.
- [ ] Убрать фильтр `Любая категория источника` из этого блока.
- [ ] Переименовать UI так, чтобы название отражало реальную функцию: например, `Массовые действия с листингами`.

### Backend

- [ ] Оставить массовое назначение адресов отдельной операцией или явно выделить его как действие `update_placement`.
- [ ] Добавить API для массовых действий над листингами:
  - `publish`
  - `archive`
  - `delete`
  - `update_placement`
- [ ] Поддержать выбор через явный список `listing_ids`.
- [ ] Поддержать выбор через безопасные фильтры: аккаунт, статус, выбранная страница/результат поиска.
- [ ] Все операции должны быть tenant-scoped.
- [ ] Валидировать статусы перед действием: нельзя публиковать уже активное, архивировать неподходящий статус, удалять то, что нельзя удалить.
- [ ] Писать логи по каждому листингу и общий лог массовой операции.
- [ ] Возвращать админу понятный результат: сколько успешно, сколько пропущено, сколько с ошибкой и почему.
- [ ] Отправлять Telegram-уведомления об успешных публикациях, если Telegram привязан к tenant-у/админу.
- [ ] Сохранить текущие Telegram-уведомления об ошибках публикации и сделать формат успешных уведомлений симметричным: аккаунт, товар/листинг, статус, ссылка на Avito при наличии.

### Очередь Avito feed

- [ ] Не перетирать активный feed новым одиночным feed-ом, пока Avito не обработал предыдущий.
- [ ] Ввести очередь публикаций по Avito-аккаунту: один активный feed-run на аккаунт, остальные листинги ждут в очереди.
- [ ] Новые массовые публикации должны добавляться в очередь и объединяться в следующую партию.
- [ ] После ответа Avito или истечения таймаута запускать следующую партию.
- [ ] В dashboard показывать понятные статусы: `В очереди`, `На модерации Avito`, `Опубликовано`, `Отклонено`, `Ошибка`.

### Frontend

- [ ] Заменить текущий блок на панель массовых действий с выбором операции.
- [ ] Оставить поля адреса/телефона/контакта только для действия назначения локации.
- [ ] Для публикации, архивации и удаления показывать подтверждение с количеством затронутых листингов.
- [ ] Для удаления использовать более строгий confirmation flow.
- [ ] После выполнения обновлять таблицу листингов и показывать итог операции.

### Тесты

- [ ] Unit/API тесты массового назначения адресов: данные листингов обновляются корректно.
- [ ] Unit/API тесты массовой публикации: листинги переходят в очередь/публикацию, создаются задачи, соблюдается tenant isolation.
- [ ] Unit/API тесты массовой архивации: статусы и логи меняются корректно.
- [ ] Unit/API тесты массового удаления: удаляются только допустимые листинги, остальные возвращаются как skipped/error.
- [ ] Тесты аккаунтов Avito: действия не смешивают листинги разных аккаунтов и tenant-ов.
- [ ] Тесты логов: по каждому массовому действию есть общий лог и результат по листингам.
- [ ] Тесты Telegram-уведомлений: успешная публикация отправляет уведомление только при привязанном Telegram, ошибки продолжают уведомляться корректно.
- [ ] Тесты feed queue: второй feed не перетирает первый, пока первый не обработан Avito или не истёк timeout.
- [ ] Тесты с mock Avito: без реальных внешних запросов.
