"""Тесты маппинга внутренней таксономии на категории Avito."""
...
class AvitoCategoryMapTest(SimpleTestCase):
    ...
    def test_all_target_slugs_exist_in_specs(self):
        ...
        for slug in list(ROOT_TO_AVITO.values()) + list(LEAF_TO_AVITO.values()):
            self.assertIn(slug, specs, f'slug {slug} отсутствует в avito_field_specs.json')
    ...
    def test_every_taxonomy_category_resolves_with_goodstype(self):
        ...
        for root in BASE_PART_CATEGORY_TREE:
            ...
            for name, parent in pairs:
                ...
                self.assertTrue(spec, f'категория «{name}» не сопоставлена с Avito')
                self.assertIn('GoodsType', spec['fixed'], f'у «{name}» ({spec["slug"]}) нет GoodsType')
    ...
    def test_leaf_override_wins_over_root(self):
        ...
        self.assertEqual(resolve_avito_slug('Фары', 'Кузов, стекла и оптика'), 'avtosvet')

    def test_special_branches_goodstype(self):
        """Шины и масла уходят в свои GoodsType, а не «Запчасти»."""
        self.assertEqual(avito_spec_for('Шины', 'Колеса и шины')['fixed']['GoodsType'], 'Шины, диски и колёса')
        self.assertEqual(
